...
export default function FloatingShare() {
  ...
  return (
    <div className="fixed left-3 sm:left-5 bottom-24 lg:bottom-6 z-40 flex flex-col-reverse items-center gap-2.5">
      {/* Main Toggle Button */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className={`w-10 h-10 rounded-full flex items-center justify-center shadow-sm transition-all duration-300 ${ isOpen ? 'bg-ink text-paper rotate-90' : 'bg-card text-ink border border-rule' }`}
      ...
      <div className={`flex flex-col gap-3 transition-all duration-300 origin-bottom ${ isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-90 translate-y-10 pointer-events-none' }`}>
        ...
        <button 
          ...
          className="w-10 h-10 rounded-full bg-card text-muted flex items-center justify-center shadow-sm border border-rule hover:-translate-y-1 hover:text-signal transition-all relative group"
          title="Copy Link"
        >
          <Link2 size={16} />
          {showCopied && (
            <span className="absolute left-full ml-3 px-2 py-1 bg-ink text-paper text-[10px] rounded block whitespace-nowrap">
              Copied!
            </span>
          )}
        </button>
