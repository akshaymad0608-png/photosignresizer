      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {CATEGORIES.map((cat, idx) => (
            <div key={idx} className="bg-card-sunk rounded-xl p-6 shadow-sm border border-rule hover:border-signal transition-colors group">
              <div className="flex items-center gap-4 mb-6">
                <div className="p-2.5 rounded-lg bg-ink text-paper">
                  {React.cloneElement(cat.icon as React.ReactElement, { className: "w-5 h-5" })}
                </div>
                <h2 className="text-xl font-bold text-ink tracking-tight">{cat.title}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                {cat.tools.map(tool => (
                  <Link 
                    key={tool.id} 
                    to={`/tools/${tool.id}`}
                    className="px-4 py-2 bg-card text-sm font-medium text-ink-soft rounded-lg hover:bg-card-sunk hover:text-signal transition-colors truncate text-center sm:text-left border border-rule"
                  >
                    {tool.name}
                  </Link>
                ))}
